...
Codex has one narrow busy-only exception while its lifecycle source remains unverified: the read-only native check reports busy only when the named server is already running, Herdr reports `working`, exact-pane process-info identifies a single foreground Codex process by both process name and argv0, and the live process table confirms that pid in the reported foreground group.
Unreadable, mismatched, ambiguous, non-Codex, or non-working evidence remains unknown; this check does not open Codex's stored-record gate, trust footer text, or verify hooks or app-server events.
