...
For Codex, `max` is model-scoped: bootstrap accepts it only with a catalog-proven selected model, while an explicit spawn with another or unproven model records `effort=max` but omits the launch flag.
