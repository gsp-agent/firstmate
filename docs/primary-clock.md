...
The UTC start time of the exact current pinned Codex process marks its owner generation because a resumed process reuses the session UUID and rollout.
Lifecycle events older than that boundary belong to an earlier owner and are ignored; events at or after it are correlated by `turn_id`, and an unmatched current-owner start is busy.
A valid rollout with prior lifecycle history but no current-owner events is idle, while a rollout with no lifecycle history remains unknown.
Process start and event timestamps are compared at whole-second precision, with equality treated as current.
Missing, malformed, duplicate, mismatched, or unsupported lifecycle evidence, including unreadable timestamps, is unknown and blocks input.
...
The clock script itself never creates tasks or queue records.
